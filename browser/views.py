...
from .filters import UserIDFilter, UsernameFilter, VideoFilter
from .models import Config, Lockcategory, Sponsortime, Username, Vipuser, Warnings
from .tables import SponsortimeTable, UserIDTable, UsernameTable, VideoTable
...
def populate_context(context, filter_args):
    ...
    context["user_hidden"] = (
        Sponsortime.objects.filter(**filter_args)
        .filter(votes__gte=-1)
        .filter(Q(hidden=1) | Q(shadowhidden__gte=1))
        .count()
    )
    ...
    context["ignored_views"] = (
        Sponsortime.objects.filter(**filter_args).filter(votes__lte=-2).aggregate(Sum("views"))["views__sum"]
    )
...
def populate_context_video_details(context, videoid):
    ...
    context["hidden"] = (
        Sponsortime.objects.filter(videoid=videoid)
        .filter(votes__gte=-1)
        .filter(Q(hidden=1) | Q(shadowhidden__gte=1))
        .count()
    )
...
class FilteredUUIDListView(SingleTableMixin, FilterView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs: Any) -> dict[str, Any]:
        ...
        context["votes"] = SponsortimeTable.render_votes(
            self.uuid.votes, FakeRecord(self.uuid.locked, self.uuid.user_id)
        )
